...
export default function Form() {
  ...
  const [firstLoadingStauts, setFirstLoadingStatus] = useState<Status>(Status.INACTIVE)
  ...
  async function fetchMoreEvents() {
    const url =
      year === ""
        ...
        : `${process.env.NEXT_PUBLIC_EVENTO_API_URL
        }/search/events?artistMbid=${selectedMbid}&year=${year}&p=${currentPage + 1
        }`;
    try {
  ...
  return (
    <div className="relative">
      ...
      {firstLoadingStauts === Status.PENDING ? (<div className="mt-4 flex flex-col justify-center items-center"><Loader /></div>)
        : (
